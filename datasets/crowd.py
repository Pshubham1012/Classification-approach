...
def gen_discrete_map(im_height, im_width, points):
    ...
    discrete_map = torch.zeros(im_width * im_height).scatter_add_(0, index=p_index.to(torch.int64), src=torch.ones(im_width*im_height)).view(im_height, im_width).numpy()
...
class Crowd_sh(Base):
    ...
    def __getitem__(self, item):
        ...
        #----------------------------------------------------------------------
        # Load CSV file
        gd_path = os.path.join(self.root_path, 'ground-truth', 'GT_{}.txt'.format(name)) #added txt here change to csv if needed
# =============================================================================
#         if self.method == 'train':
#             gd_path = os.path.join(self.root_path, 'ground-truth', '{}.csv'.format(name))
#         else:
#             gd_path = os.path.join(self.root_path, 'ground-truth', 'GT_{}.csv'.format(name)) #added
# =============================================================================
        #keypoints_csv = np.loadtxt(gd_path, delimiter=',')  #added
...
#             keypoints_csv = np.loadtxt(gd_path, delimiter=',')
#         except UserWarning:
#             # If the file is empty, create an empty NumPy array to store the keypoints
#             keypoints_csv = np.empty((0, 2))
# =============================================================================
        #--------------------------------------------------------------------
        # Convert to NumPy array
        keypoints = np.array(keypoints_csv) #added
        
        print(img_path,gd_path)
