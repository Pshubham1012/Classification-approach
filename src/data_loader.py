    ...
    def __init__(self, data_path, gt_path, shuffle=False, gt_downsample=False, pre_load=False):
        ...
        if self.pre_load:
            ...
            for fname in self.data_files:
                ...
                den = pd.read_csv(os.path.join(self.gt_path,"GT_"+os.path.splitext(fname)[0] + '.txt'), sep=',',header=None).to_numpy()#as_matrix()                        
    ...
    def __iter__(self):
        ...
        for idx in id_list:
            if self.pre_load:
                ...
                img = cv2.resize(img,(wd_1,ht_1))
                img = img.reshape((1,1,img.shape[0],img.shape[1]))
                den = pd.read_csv(os.path.join(self.gt_path,'GT_'+os.path.splitext(fname)[0] + '.txt'), sep=',',header=None).as_matrix()                        
                den  = den.astype(np.float32, copy=False)
                if self.gt_downsample:
                    wd_1 = wd_1/4
                    ht_1 = ht_1/4
