...
class Trainer(object):
    ...
    def setup(self):
        ...
        downsample_ratio = 8
		#so in the following define same things for second variable or input
        if args.dataset.lower() == 'qnrf':
            ...
            self.datasets = {'train': Crowd_sh(os.path.join(args.data_dir, 'train_data'),
                             ...
                             'val': Crowd_sh(os.path.join(args.data_dir, 'validation'),
    ...
    def train_eopch(self):
        ...
        for step, (inputs, points, gt_discrete) in enumerate(self.dataloaders['train']):   #make changes here, make input1,input2
            ...
            with torch.set_grad_enabled(True):
                outputs, outputs_normed = self.model(inputs) #then give those input1 and input2 here
                # Compute OT loss.
                #start_time = time.time()  #extra added
                ot_loss, wd, ot_obj_value = self.ot_loss(outputs_normed, outputs, points)
                #end_time = time.time()  #extra added
                #print(f"Time taken for forward pass of loss function: {end_time - start_time:.4f} seconds") #extra added
    ...
    def val_epoch(self):
        ...
        if (2.0 * mse + mae) < (2.0 * self.best_mse + self.best_mae):
            ...
            self.logger.info("save best mse {:.2f} mae {:.2f} model epoch {}".format(self.best_mse,
                                                                                     self.best_mae,
                                                                                     self.epoch))
            torch.save(model_state_dic, os.path.join(self.save_dir, 'best_mse_{:.2f}_mae_{:.2f}_model_epoch_{}_best_model_{}.pth'.format(self.best_mse,
                                                                                     self.best_mae,
                                                                                     self.epoch,self.best_count)))
            self.best_count += 1
